...
HELM_CHART_NAME	:= ${project.title?lower_case}
...
docker-build: guard-BUILD_USER guard-BUILD_PASSWORD guard-HTTP_PROXY guard-VERSION
  ...
  # TODO: Add your docker repository
	...
	docker build --no-cache -t <YOUR-DOCKER-REPO>/${project.title?lower_case}:$(VERSION) --build-arg http_proxy=$(HTTP_PROXY) --build-arg https_proxy=$(HTTP_PROXY) .
...
docker-push: guard-BUILD_USER guard-BUILD_PASSWORD
  ...
  # TODO: Push the build container with the tag from docker-build
	...
	docker push <YOUR-DOCKER-REPO>/${project.title?lower_case}:$(VERSION)